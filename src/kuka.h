...
#define setServoTurn(id,dir,speed) uKitServo.setServoTurn1M(id,dir,speed)//舵机轮模式控制，id是舵机号，dir是方向（1顺时针，0逆时针），speed是速度（0-5）
#define setServoAngle(id,angle,times) uKitServo.setServoAngle1M(id,angle,times)//舵机舵机模式，id是舵机号，angle是角度（-118°~118°），times是运行时间（300-5000）
#define setServoStop(id) uKitServo.setServoStop1M(id)//单个舵机停止函数
#define readServoAnglePD(id) uKitServo.readServoAnglePD1M(id)//单个舵机回读，返回舵机角度值(掉电回读）
#define readServoAnglePD_M(read_id,num) uKitServo.readServoAnglePD_M(read_id,num)//单个舵机回读，返回舵机角度值(掉电回读）
#define readServoAngleNPD(id) uKitServo.readServoAngleNPD1M(id)//单个舵机回读，返回舵机角度值(不掉电回读）
#define readServoAngleNPD_M(read_id,num) uKitServo.readServoAngleNPD_M(read_id,num)//单个舵机回读，返回舵机角度值(掉电回读）
#define playMotion(id,action,times) uKitServo.playMotion1M(id,action,times)//播放动作
//uKitSensor_API
    ...
    case 8: //巡线传感器
     ...
     case 11:    //ID相关
      ...
      case 130: //停止设备
        ...
        case 127: //修改ID               
           root["mode"]=127;
           uKitId.setAllDeciveId1M(buf[0],buf[1],buf[2]);
           delay(120);                
           if(buf[2]==uKitId.getAllDeciveId1M(buf[0],buf[2])){
            root["code"]=0;
           }
           else{
            root["code"]=1;
